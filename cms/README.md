...
5. Run `npm run deploy` to create and publish the Studio at its separate `*.sanity.studio/newsroom` address. On the first deployment, copy the generated app ID into `.env` as `SANITY_STUDIO_APP_ID`, then deploy once more to enable managed auto-updates. Do not link that address from the public website.
